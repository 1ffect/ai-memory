...
- 九个研究板块：总览、概念、架构、工作流、更新、记忆演进、开源案例、机制对照、评测方法。
- 四张 SVG 示意图，点击节点显示解析。
- 工作流播放、暂停、逐步查看与重置。
- 七种新旧记忆更新案例与前后对比。
- 上下文预算滑杆与三种加载策略比较。
- 十五个开源案例，支持分类切换、九维架构剖面、逐步流程播放和来源链接。
- 五类架构原型图、六层评测链路，以及 LoCoMo、LongMemEval、MemoryAgentBench、LongMemEval-V2 的边界对照。
...
| `site/research-data.js` | 十五个开源案例的分析、架构剖面与来源 |
...
JavaScript 语法、九个页面内容生成、图解节点与连线引用已检查，并完成桌面与移动端的浏览器验收。GitHub Pages 的线上状态仍以仓库工作流为准。
...
已暂移除产品能力与验证风险章节，保留基础机制与图解；案例库覆盖 Qwen-Agent、Kimi Code CLI、OpenViking、ReMe、MemoryOS、MemOS、Cognee、A-MEM、Memobase、Generative Agents、LazyMem 等十五个公开项目。各案例明确官方归属、公开证据范围和消费端产品推断边界。
